...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, fx, fy, cx, cy, image, image_name, uid,
                 ...
                 height=None, width=None, depth=None, normal=None, image_mask=None, lab=None):
        ...
        if lab is not None:
            self.lab = lab.to(self.data_device)
        else: 
            self.lab = torch.zeros((3, self.image_height, self.image_width), dtype=torch.float32, device=data_device)
        ...
        self.splatted_depth = torch.zeros_like(self.depth)
        # self.reflectance = torch.zeros_like(self.original_image)
        self.reflectance = self.original_image
        self.shading = torch.zeros_like(self.depth)
        self.residual = torch.zeros_like(self.original_image)
        ...
        if self.fx is None:
            ...
            self.projection_matrix = getProjectionMatrixCenterShift(
                self.znear, self.zfar, cx, cy, fx, fy, self.image_width, self.image_height).transpose(0, 1).to(self.data_device)
    ...
    def get_primary_axis(self):
        p_axis = torch.zeros([3], dtype=torch.float32).to(self.data_device)
        p_axis[2] = 1
    ...
    def get_rotation(self):
        return torch.from_numpy(self.R.T).float().to(self.data_device)

    @staticmethod
    def create_for_gui():
        return Camera(0, np.eye(3), np.zeros(3), 50, 50, None, None, "gui", "gui")

    def unproject(self, gt_depth=False, splatted_depth=None):
        
        assert not ((not gt_depth) and splatted_depth is None), "不使用gt深度但是splat深度为空"
        
        if gt_depth:
            depth = self.depth[0]
        else:
            depth = splatted_depth[0]
        depth = depth.to(self.data_device) 
        tmp1, tmp2 = torch.meshgrid(torch.arange(self.image_height), torch.arange(self.image_width))
        pixel_pos = torch.stack([tmp2, tmp1], dim=-1).float().to(self.data_device)
        camera_pos_x = (pixel_pos[:, :, 0] - self.intrinsics[0, 2]) / self.intrinsics[0, 0] * depth
        camera_pos_y = (pixel_pos[:, :, 1] - self.intrinsics[1, 2]) / self.intrinsics[1, 1] * depth
        camera_pos_z = depth * torch.ones_like(camera_pos_x)
        camera_pos = torch.stack([camera_pos_x, camera_pos_y, camera_pos_z], dim=-1)
        world_pos = torch.matmul(camera_pos - self.extrinsics[:3, 3], self.extrinsics[:3, :3])
        return world_pos 