...
def load_img(path):
    ...
    #     # laplacian_img = get_laplacian_image_np(img)
    #     img = img / 255
...
def load_depth_png(depth_path):
    ...
    depth = depth.astype(np.float32) / 1000.0  # uint16 mm depth, then turn depth from mm to meter
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, debug=False, have_masks=False):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        if not os.path.exists(image_path):
            continue
        image, is_hdr, lab = load_img(os.path.join(images_folder, image_name))
        ...
        depth = None
        # depth_path = os.path.join(os.path.dirname(images_folder), "depth", os.path.basename(extr.name).replace("rgb", "depth").replace("JPG", "npy"))
        depth_path = os.path.join(os.path.dirname(images_folder), "depths", os.path.basename(extr.name).replace("rgb", "depth").replace("jpg", "png"))
        ...
        assert depth is not None, f"Depth not found for {depth_path}" 
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovX=Fovx, FovY=FovY, fx=focal_length_x, fy=focal_length_y, cx=ppx,
                              ...
                              image_path=image_path, image_name=image_name, width=width, height=height, hdr=is_hdr, lab=lab)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", debug=False): # nerf
    cam_infos = []
...
sceneLoadTypeCallbacks = {
    "Colmap": readColmapSceneInfo, # replica
    "Blender": readNerfSyntheticInfo, # invrender
    "NeILF": readNeILFInfo,
    'nyu': readNerfSyntheticInfo,
}
