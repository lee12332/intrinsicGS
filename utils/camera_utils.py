...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    image = torch.from_numpy(cam_info.image).float().permute(2, 0, 1) # 3 h w
    ...
    resized_lab = None#LAb颜色空间的图像数据，按理说应该是从rgb那转过来的，但是为了不每次都算，直接转了放在数据集中读
    if cam_info.lab is not None:
        lab = torch.from_numpy(cam_info.lab).float().permute(2, 0, 1)
        resized_lab = torchvision.transforms.Resize(
            resolution, interpolation=InterpolationMode.NEAREST)(lab)
    ...
    # cam1= Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
    ...
    #               image_name=cam_info.image_name, uid=id, data_device=args.data_device, hdr=cam_info.hdr, lab = resized_lab)
    # print("sizeof camera:",sys.getsizeof(cam1))
